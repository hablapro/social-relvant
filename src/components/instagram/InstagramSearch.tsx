...
export default function InstagramSearch() {
  const { query, updateQuery } = useSearchPersistence('instagram');
  ...
  const performSearch = async () => {
    ...
    try {
      const cleanUsername = query.trim().startsWith('@') 
        ? query.trim().slice(1) 
        : query.trim();
      ...
      <div className="w-full max-w-2xl mx-auto space-y-4">
        <form onSubmit={handleSubmit} className="flex gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-6 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 disabled:opacity-50 flex items-center gap-2"
          >
            {loading ? (
